...
def craft_one_type(sess, model, X, Y, dataset, attack, batch_size, ind_bat):
    """
    TODO
    :param sess:
    :param model:
    ...
    sio.savemat('../data/Adv_%s_%s_%s.mat' % (args.dataset, args.attack, ind_bat),adict)
    np.save('../data/Adv_%s_%s_%s.npy' % (args.dataset, args.attack, ind_bat), X_adv)
    ...
    if args.attack == 'all':
        ...
        for attack in ['fgsm', 'bim-a', 'bim-b', 'jsma', 'cw']:
            craft_one_type(sess, model, X_test, Y_test, args.dataset, attack,
                           args.batch_size,ind_bat)
    else:
        # Craft one specific attack type
        ind_bat = 1
        craft_one_type(sess, model, X_test[:2500], Y_test[:2500], args.dataset, args.attack,
                       args.batch_size,ind_bat)
        ind_bat = 2
        craft_one_type(sess, model, X_test[2500:5000], Y_test[2500:5000], args.dataset, args.attack,
                       args.batch_size,ind_bat)
    print('Adversarial samples crafted and saved to data/ subfolder.')
    import scipy.io as sio 
    Data1 = sio.loadmat('../data/Adv_%s_%s_1.mat' % (args.dataset, args.attack))
    X_1 = Data1['X']
    Y_1 = Data1['Y']
    X_adv1 = Data1['X_adv']
    Y_preds1 = Data1['Y_preds']
    X_encoder_1 = Data1['X_encoder']
    X_adv_encoder_1 = Data1['X_adv_encoder']
    Data2 = sio.loadmat('../data/Adv_%s_%s_2.mat' % (args.dataset, args.attack))
    X_2 = Data2['X']
    Y_2 = Data2['Y']
    X_adv2 = Data2['X_adv']
    Y_preds2 = Data2['Y_preds']
    X_encoder_2 = Data2['X_encoder']
    X_adv_encoder_2 = Data2['X_adv_encoder']

    X = np.concatenate((X_1,X_2),axis=0)
    Y = np.concatenate((Y_1,Y_2),axis=0)
    X_adv = np.concatenate((X_adv1,X_adv2),axis=0)
    Y_preds = np.concatenate((Y_preds1,Y_preds2),axis=0)
    X_encoder = np.concatenate((X_encoder_1,X_encoder_2),axis=0)
    X_adv_encoder = np.concatenate((X_adv_encoder_1,X_adv_encoder_2),axis=0)

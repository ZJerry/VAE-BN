function [log_T2_normal,log_T2_test,S,V] = comput_log_T2_MNIST_normal_train_test(x_train_recon,x_test_recon,x_train,x_test)
gtsrb = 0;mnist = 1;cifar = 0;
...
elseif cifar
    ...
    NumVari = 784; NumTrain = 60000; Numtest=5000;
end
...
x= X_Normal;%   Traning data, normal operation
[NumSamp,NumVari]= size(X_Normal);
[U,S,V,flag]= svds(double(x)/sqrt(NumSamp-1),NumVari);%   singular value decomposition, x/sqrt(n-1)=USV'
% load SV_mnist  % Load the singular value set pre-computed by svds from normal training data
